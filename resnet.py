...
def initialize_weight(layer):
    if isinstance(layer, torch.nn.Linear) or isinstance(layer, torch.nn.Conv2d): torch.nn.init.kaiming_normal_(layer.weight)
...
class StiffnessLoss:
    def __init__(self, lagrange, batch_size):
